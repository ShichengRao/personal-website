...
create or replace function create_game(p_code text, p_seed integer, p_name text, p_token text)
returns text language plpgsql security definer set search_path = public as $$
begin
  if p_code is null or p_code !~ '^[a-z]+(-[a-z]+){2}$' or length(p_code) > 40 then
    raise exception 'bad game id';
  end if;
  if p_seed is null then raise exception 'bad seed'; end if;
...
revoke all on function create_game(text, integer, text, text) from public;
...
grant execute on function create_game(text, integer, text, text) to anon, authenticated;
